...
def main():
    ...
    train_set = BaseDataset(
        data_df=train_df, tokenizer=tokenizer, token_max_len=CONFIGS["SESSION"]["INPUT_MAX_LEN"], e_tokens=CONFIGS["DATA"]["ENTITY_TOKEN"]
    )
    valid_set = BaseDataset(
        data_df=valid_df, tokenizer=tokenizer, token_max_len=CONFIGS["SESSION"]["INPUT_MAX_LEN"], e_tokens=CONFIGS["DATA"]["ENTITY_TOKEN"]
    )
